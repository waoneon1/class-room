...
<div class="mb-6">
    ...
    <h1 class="text-xl font-bold text-gray-800">Edit Materi</h1>
</div>

<div class="bg-white rounded-2xl shadow-sm p-6 max-w-xl">
    <form method="POST" action="{{ route('guru.materi.update', $materi) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="mata_pelajaran_id"
                    ...
                    @foreach($mapel as $m)
                        <option value="{{ $m->id }}" {{ old('mata_pelajaran_id', $materi->mata_pelajaran_id) == $m->id ? 'selected' : '' }}>{{ $m->nama_pelajaran }}</option>
                    @endforeach
                </select>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    ...
                    <label class="flex items-center gap-2 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                        <input type="checkbox" name="kelas_id[]" value="{{ $k->id }}"
                            ...
                            {{ (is_array(old('kelas_id')) && in_array($k->id, old('kelas_id'))) || (!old('kelas_id') && $materi->kelas->contains($k->id)) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3"
                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary @error('deskripsi') border-red-400 @enderror">{{ old('deskripsi', $materi->deskripsi) }}</textarea>
                @error('deskripsi')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">File Materi</label>
                ...
                <div class="bg-gray-50 rounded-xl px-4 py-2.5 mb-2 text-sm text-gray-600 flex items-center gap-2">
                    ...
                    File saat ini: {{ basename($materi->file_materi) }}
                </div>
                @endif
