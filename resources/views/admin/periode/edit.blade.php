...
<div class="mb-6">
    ...
    <h1 class="text-xl font-bold text-gray-800">Edit Periode</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.periode.update', $periode) }}">
        ...
        <div class="mb-4">
            <label class="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" class="rounded text-primary focus:ring-primary h-4 w-4" {{ old('is_active', $periode->is_active) ? 'checked' : '' }}>
                Jadikan Periode Aktif
            </label>
            <p class="text-xs text-gray-500 mt-1">Hanya 1 periode yang bisa aktif. Jika Anda centang, periode lain akan otomatis dinonaktifkan.</p>
        </div>
        <div class="flex gap-3 mt-6">
            <button type="submit" class="bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition-colors">Simpan Perubahan</button>
